...
- 🛰 **Live view** — on a picked train, tap “Live view” for a full-screen screen with a big live countdown, its status (on time / late / cancelled / direction / next stop), the door-to-door details, and **only that train moving on the radar map** (auto-followed by run number, or tap it to lock on). The delay/cancellation shown here comes from the **geOps live feed** (streamed up from the embedded map), so it’s real-time even when the timetable API reports nothing. Reuses the geOps radar in an embedded frame; the deployed site has the geOps key built in at deploy time (see `.github/scripts/inject-keys.js`), so the live map just works — if you run it yourself without that secret, add a free key once via the radar page.
